...
const Group= (props: any, id: number) => {
  ...
  const { data: invitations, error } = useSWR(
    "http://52.195.64.253:3010/api/v1/invitations",
    fetcher
  );
  ...
  const handleDeleteUserInGroup = async (userId: number) => {
    ...
    const cookieData = getCookie();
    try {
      const response = await fetch(`http://52.195.64.253:3010/api/v1/users/change_group`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleInvite = async () => {
    ...
    const cookieData = getCookie();
    try {
      const response = await fetch(`http://52.195.64.253:3010/api/v1/invitations`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleApproveInvitation = async (invitationId: number) => {
    ...
    const cookieData = getCookie();
    try {
      const response = await fetch(`http://52.195.64.253:3010/api/v1/users/approve_group`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      router.push('/');
      mutate('http://52.195.64.253:3010/api/v1/invitations');
    } catch (error) {
      console.error('An error occurred:', error);
